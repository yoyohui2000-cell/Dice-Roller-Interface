...
export function parseTurnState(text: string): { cleanText: string; turnState: TurnState } {
  const defaultState: TurnState = { who: "全體", dice: null, purpose: null };
  const marker = /\n?%%TURN:(\{[^%]+\})%%\s*$/s;
  const match = text.match(marker);
  if (!match) return { cleanText: text.trimEnd(), turnState: defaultState };
  try {
    const turnState = JSON.parse(match[1]) as TurnState;
    const cleanText = text.replace(marker, "").trimEnd();
    return { cleanText, turnState };
  } catch {
    return { cleanText: text.trimEnd(), turnState: defaultState };
  }
}
