...
export type PlayerUpdate = {
  ...
  deathSaveSuccess: number | null;
  deathSaveFailure: number | null;
  stabilize: boolean;
};
...
# 角色狀態更新標記（選擇性，有明確變化時才輸出）
當GM敘事導致玩家角色狀態發生明確、具體的變化時，在%%TURN%%標記之前輸出以下標記：
%%PLAYER_UPDATE:{"name":"[角色完整名稱]","hpChange":[整數或null],"conditionsAdd":["狀態"],"conditionsRemove":["狀態"],"inventoryAdd":[{"name":"物品名","qty":數量}],"inventoryRemove":["物品名"],"spellSlotsUse":[{"level":環級,"count":數量}],"deathSaveSuccess":[整數或null],"deathSaveFailure":[整數或null],"stabilize":[true或false]}%%

規則：
- name: 角色的完整名稱（與角色卡一致）
- hpChange: 正數=治癒/回復HP，負數=受傷/扣血，null=無HP變化
- conditionsAdd: 新增的狀態效果（如["中毒","倒地"]），無變化則用空陣列[]
- conditionsRemove: 移除的狀態效果，無則用[]
- inventoryAdd: 新獲得物品（如[{"name":"治癒藥水","qty":2}]），無則用[]
- inventoryRemove: 消耗/失去物品的名稱（如["箭矢","火炬"]），無則用[]
- spellSlotsUse: 使用的法術位（如[{"level":1,"count":1}]），無則用[]
- deathSaveSuccess: 本次新增的死亡豁免成功數（通常為1），null=無變化
- deathSaveFailure: 本次新增的死亡豁免失敗數（通常為1），null=無變化
- stabilize: true=角色穩定化（3次成功或被治癒），重置死亡豁免並讓HP回到1；false=無
- 每個受影響的玩家輸出一個此標記
- 只在有實際變化時才輸出，普通對話/移動/無傷害的場景描述不需輸出
- 此標記不顯示在敘事中，是系統控制用

# 死亡豁免規則（重要）
玩家HP降至0時進入「瀕死」狀態，每回合必須擲D20死亡豁免：
- 結果10以上：成功（累積3次成功=穩定化）
- 結果9以下：失敗（累積3次失敗=死亡）
- 結果20：立即穩定化並回到1HP
- 結果1：計兩次失敗
HP降至0時：加入conditionsAdd:["瀕死"]
穩定化時（3次成功/治療）：stabilize:true，移除conditionsRemove:["瀕死"]，hpChange讓HP回到1
3次失敗時：conditionsAdd:["死亡"]，移除conditionsRemove:["瀕死"]

範例（玩家受傷並中毒）：
%%PLAYER_UPDATE:{"name":"勇者小明","hpChange":-8,"conditionsAdd":["中毒"],"conditionsRemove":[],"inventoryAdd":[],"inventoryRemove":["治癒藥水"],"spellSlotsUse":[],"deathSaveSuccess":null,"deathSaveFailure":null,"stabilize":false}%%
%%TURN:{"who":"勇者小明","dice":null,"purpose":null}%%

範例（玩家HP降至0）：
%%PLAYER_UPDATE:{"name":"勇者小明","hpChange":-99,"conditionsAdd":["瀕死"],"conditionsRemove":[],"inventoryAdd":[],"inventoryRemove":[],"spellSlotsUse":[],"deathSaveSuccess":null,"deathSaveFailure":null,"stabilize":false}%%
%%TURN:{"who":"勇者小明","dice":"D20","purpose":"死亡豁免"}%%

範例（死亡豁免成功一次）：
%%PLAYER_UPDATE:{"name":"勇者小明","hpChange":null,"conditionsAdd":[],"conditionsRemove":[],"inventoryAdd":[],"inventoryRemove":[],"spellSlotsUse":[],"deathSaveSuccess":1,"deathSaveFailure":null,"stabilize":false}%%

範例（穩定化）：
%%PLAYER_UPDATE:{"name":"勇者小明","hpChange":1,"conditionsAdd":[],"conditionsRemove":["瀕死"],"inventoryAdd":[],"inventoryRemove":[],"spellSlotsUse":[],"deathSaveSuccess":null,"deathSaveFailure":null,"stabilize":true}%%
...
export function parsePlayerUpdates(text: string): { cleanText: string; playerUpdates: PlayerUpdate[] } {
  const playerUpdates: PlayerUpdate[] = [];
  const pattern = /\n?%%PLAYER_UPDATE:(\{[^%]*\})%%[ \t]*/g;
  let m: RegExpExecArray | null;
  while ((m = pattern.exec(text)) !== null) {
    try {
      const raw = JSON.parse(m[1]) as Partial<PlayerUpdate>;
      if (typeof raw.name === "string") {
        playerUpdates.push({
          name: raw.name,
          hpChange: typeof raw.hpChange === "number" ? raw.hpChange : null,
          conditionsAdd: Array.isArray(raw.conditionsAdd) ? raw.conditionsAdd : [],
          conditionsRemove: Array.isArray(raw.conditionsRemove) ? raw.conditionsRemove : [],
          inventoryAdd: Array.isArray(raw.inventoryAdd) ? raw.inventoryAdd : [],
          inventoryRemove: Array.isArray(raw.inventoryRemove) ? raw.inventoryRemove : [],
          spellSlotsUse: Array.isArray(raw.spellSlotsUse) ? raw.spellSlotsUse : [],
          deathSaveSuccess: typeof raw.deathSaveSuccess === "number" ? raw.deathSaveSuccess : null,
          deathSaveFailure: typeof raw.deathSaveFailure === "number" ? raw.deathSaveFailure : null,
          stabilize: raw.stabilize === true,
        });
      }
    } catch {
      // malformed — skip
  ...
  return { cleanText, playerUpdates };
}

export function parseTurnState(text: string): { cleanText: string; turnState: TurnState; combatState?: CombatState; playerUpdates: PlayerUpdate[] } {
  const defaultState: TurnState = { who: "全體", dice: null, purpose: null };
  let workingText = text;
  let combatState: CombatState | undefined;

  // Strip %%PLAYER_UPDATE%% markers first and collect them
  const { cleanText: textAfterPlayerUpdates, playerUpdates } = parsePlayerUpdates(workingText);
  workingText = textAfterPlayerUpdates;
  ...
  return { cleanText, turnState, combatState, playerUpdates };
}
